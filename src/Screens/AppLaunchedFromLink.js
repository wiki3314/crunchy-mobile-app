  ...
  const getRestaurantDetails = async () => {
    try {
      ...
      if (restaurantDetails?.photos && restaurantDetails.photos.length > 0) {
        setRestaurantImage(
          `https://maps.googleapis.com/maps/api/place/photo?maxwidth=${windowWidth}&photo_reference=${restaurantDetails.photos[0].photo_reference}&key=${GOOGLE_API_KEY}`
        );
        let arrGoogleImages = [];
    ...
    if (item.isInAppPost) {
      if (item.mediaType == "image") {
        return (
          ...
          >
            <TouchableOpacity
              onPress={onCloseModalIconPress}
              style={styles.inAppPostImageCloseIcon}
            >
        ...
        return (
          <View style={commonStyles.flexFull}>
            <TouchableOpacity
              onPress={onCloseModalIconPress}
              style={styles.inAppPostVideoCloseIcon}
            >
              <AntDesign name="close" style={styles.closeIcon} />
            </TouchableOpacity>
            <Video
      ...
      return (
        <ImageBackground
          ...
          resizeMode="cover"
          source={{
            uri: `https://maps.googleapis.com/maps/api/place/photo?maxwidth=${windowWidth}&photo_reference=${item.photo_reference}&key=${GOOGLE_API_KEY}`,
          }}
        >
          <TouchableOpacity
            onPress={() => {
            ...
            style={styles.googleMediaCloseIcon}
          >
            <AntDesign name="close" style={styles.closeIcon} />
          </TouchableOpacity>
          <SliderDots
  ...
  const onPhoneButtonPress = () => {
    if (restaurantPhoneNumber) {
      ...
      if (Platform.OS === "android") {
        phoneNumber = `tel:${restaurantPhoneNumber}`;
      } else {
        phoneNumber = `telprompt:${restaurantPhoneNumber}`;
      }
    ...
    if (mediaItem.isInAppPost) {
      ...
      return (
        ...
        >
          <Image
            source={{
              uri: `https://maps.googleapis.com/maps/api/place/photo?maxwidth=${windowWidth}&photo_reference=${mediaItem.photo_reference}&key=${GOOGLE_API_KEY}`,
            }}
            style={styles.singleMediaComponent(mediaIndex == 0)}
          />
        </TouchableOpacity>
    ...
    >
      ...
      >
        <View style={commonStyles.flexFull}>
                          ...
                          return (
                            innerItem <= Math.round(item.rating) && (
                              <Image
                                ...
                                style={styles.singleRatingImage}
                              />
                            )
                          );
                        })}
          ...
          <TouchableOpacity
            onPress={onContinueToApplicationPress}
            style={styles.continueButtonFullContainer}
          >
...
const styles = StyleSheet.create({
  ...
  websiteButtonContainer: (bool) => {
    return {
      ...
      flexDirection: "row",
      alignItems: "center",
      backgroundColor: bool ? colors.darkGrey : colors.appPrimary,
      borderRadius: moderateScale(8),
    };
  },
  mediaFullContainer: {
  ...
  singleMediaComponent: (bool) => {
    return {
      ...
      borderRadius: moderateScale(8),
      overflow: "hidden",
      marginLeft: bool ? 0 : moderateScale(4),
    };
  },
