...
import { FlatList, Image, ImageBackground, Modal, SafeAreaView, ScrollView, StyleSheet, Text, TouchableOpacity, TouchableWithoutFeedback, Vibration, View } from 'react-native';
import { commonStyles } from '../Constants/commonStyles';
import { moderateScale, POSTS_IMAGE_BASE_URL, ratingsData, USER_PROFILE_BASE_URL, windowHeight, windowWidth } from '../Constants/globalConstants';
...
import { setLoadNewPosts, setLoadUserData, setUserData, updateFavoriteRestaurants } from '../Redux/actions/actions';
...
export default function ProfileScreen(props) {
    ...
    function renderRestaurent(item, index) {
        return <TouchableOpacity onPress={() => {
            ...
            key={index}
        >
            <ImageBackground style={{ height: moderateScale(70), width: (windowWidth * 0.5) - moderateScale(12), borderRadius: 12, overflow: 'hidden', alignItems: 'center', justifyContent: 'center', marginTop: moderateScale(4) }} source={{ uri: item.restaurantImage }}>
    ...
    const renderDoneButton = () => {
        return <TouchableOpacity onPress={showHidePostMedia} style={{ padding: moderateScale(6), borderRadius: moderateScale(6), backgroundColor: colors.appPrimary }}>
    ...
    const renderSingleMedia = ({ item, index }) => {
        return <View style={commonStyles.flexFull}>
            <Ionicons name='close' style={{ fontSize: moderateScale(20), color: colors.black, position: 'absolute', top: moderateScale(4), right: moderateScale(6), zIndex: 99 }} onPress={showHidePostMedia} />
    ...
    return (
        <SafeAreaView style={[commonStyles.flexFull, { backgroundColor: currentThemePrimaryColor }]}>
            <ScrollView showsVerticalScrollIndicator={false} style={commonStyles.flexFull}>
                <View style={commonStyles.flexFull}>
                    ...
                    <ImageBackground resizeMode='stretch' source={isDarkModeActive ? imagePath.darkRestaurantBG : imagePath.restaurantBG} style={{ borderBottomLeftRadius: moderateScale(20), borderBottomRightRadius: moderateScale(20), overflow: 'hidden', minHeight: moderateScale(70), width: windowWidth }}>
                        <View style={styles.headerFullContainer}>
                            <View style={{ flex: 1, alignItems: 'center' }}>
                                <Text style={commonStyles.textWhite(20, { color: currentThemeSecondaryColor, marginLeft: moderateScale(22), fontWeight: 'bold', alignSelf: 'center' })}>
                                    My Profile
                                </Text>
                            </View>
                            <Ionicons onPress={onEditProfilePress} name='settings' style={{ fontSize: moderateScale(12), color: currentThemeSecondaryColor, marginRight: moderateScale(10) }} />
                        </View>
                        <View style={{ height: moderateScale(64), width: moderateScale(64), borderRadius: moderateScale(32), alignSelf: 'center', backgroundColor: currentThemeSecondaryColor, justifyContent: 'center', alignItems: 'center' }}>
                            {/* <Image style={{ height: moderateScale(62), width: moderateScale(62), borderRadius: moderateScale(31), overflow: 'hidden' }} source={userDetails?.image ? { uri: USER_PROFILE_BASE_URL + userDetails?.image } : imagePath.dummyProfile} /> */}
                        ...
                        <Text style={commonStyles.textWhite(32, { alignSelf: 'center', marginTop: moderateScale(4), color: currentThemeSecondaryColor })}>
                            {userDetails?.full_name}
                        </Text>
                        <View style={styles.buttonsFullContainer}>
                            <TouchableOpacity onPress={() => {
                                onSingleOptionPress('Followers')
                            }} style={{ justifyContent: 'center', alignItems: 'center', flexDirection: 'row', borderWidth: moderateScale(0.7), borderColor: currentThemeSecondaryColor, borderRadius: moderateScale(12), paddingVertical: moderateScale(4), paddingHorizontal: moderateScale(8) }}>
                                <Text style={commonStyles.textWhite(18, { fontWeight: '600', color: currentThemeSecondaryColor })}>
                                    Followers :
                                </Text>
                                <Text style={commonStyles.textWhite(18, { fontWeight: '600', marginLeft: moderateScale(2), color: currentThemeSecondaryColor })}>
                                    {userDetails && userDetails.followers && userDetails.followers.length > 0 ? userDetails.followers.length : 0}
                                </Text>
                            </TouchableOpacity>
                            <TouchableOpacity onPress={() => {
                                ...
                                <Text style={commonStyles.textWhite(18, { fontWeight: '600', marginLeft: moderateScale(2), color: currentThemeSecondaryColor })}>
                                    {userDetails && userDetails.following && userDetails.following.length > 0 ? userDetails.following.length : 0}
                                </Text>
                            </TouchableOpacity>
                        </View>
                    </ImageBackground>
                    <View style={{ flexDirection: 'row', marginLeft: moderateScale(12), marginTop: moderateScale(12), alignItems: 'center' }}>
                        <Text style={commonStyles.textWhite(22, { marginTop: moderateScale(2), color: currentThemeSecondaryColor })}>
                            Bio
                        </Text>
                        {!editBio ?
                            <Ionicons name={'pencil'} style={{ fontSize: moderateScale(10), color: currentThemeSecondaryColor, marginLeft: moderateScale(6) }} onPress={onEditBioIconPress} />
                            :
                            <TouchableOpacity onPress={onSaveBioPress} style={{ height: moderateScale(12), width: moderateScale(22), justifyContent: 'center', alignItems: 'center', borderRadius: moderateScale(4), marginLeft: moderateScale(4), borderWidth: moderateScale(0.7), borderColor: colors.appPrimary }}>
                                <Text style={commonStyles.textWhite(12, { color: colors.appPrimary })}>
                                    Save
                                </Text>
                            </TouchableOpacity>
                        }
                    ...
                    {!editBio ?
                        <Text style={commonStyles.textWhite(16, { fontWeight: '600', margin: moderateScale(8), alignSelf: 'center', textAlign: 'center', width: (windowWidth * 0.8) - moderateScale(19), color: currentThemeSecondaryColor, marginBottom: moderateScale(4) })}>
                            {userBio}
                        </Text>
                        :
                        <TextInput
                            ref={bioTextInputRef}
                            style={[commonStyles.textWhite(16, { fontWeight: '600', margin: moderateScale(8), marginLeft: moderateScale(12), width: (windowWidth * 0.8) - moderateScale(19), color: currentThemeSecondaryColor }), { borderWidth: moderateScale(0.7), minHeight: moderateScale(25), borderRadius: moderateScale(6), borderColor: currentThemeSecondaryColor, padding: moderateScale(4) }]}
                            value={userBio}
                ...
                <Modal visible={showPostMedia} transparent={true} >
                    <TouchableOpacity style={{ flex: 1, backgroundColor: '#000000aa', justifyContent: 'center', alignItems: 'center' }} onPress={showHidePostMedia}>
                ...
                {
                    showRestaurantOptions && <ConfirmationModal onConfirmButtonPress={onUnlikeRestaurantConfirm} onCancelPress={showHideRestaurantOptions} modalTitle={`Unlike ${selectedRestaurant.restaurantName}?`} modalDescription={`Are you sure you want to unlike ${selectedRestaurant.restaurantName}?`} />
                }
                {
                    showDeletePostModal && <DeletePostModal onCancelPress={() => {
                        setShowDeletePostModal(false)
            ...
            </ScrollView>
            {
                isDarkModeActive && <LinearGradient
                    ...
                    colors={['#ffffff00', '#ffffff04', '#ffffff09', '#ffffff0c', '#ffffff10', '#ffffff15', '#ffffff19']}
                />
            }
        </SafeAreaView>
    )
