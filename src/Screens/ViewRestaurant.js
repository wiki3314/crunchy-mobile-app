...
export default function ViewRestaurant(props) {
      ...
      if (restaurantDetails?.photos && restaurantDetails.photos.length > 0) {
        ...
        arrGoogleImages = restaurantDetails.photos.map(
          (imageItem, imageIndex) => {
            return {
              uri: `https://maps.googleapis.com/maps/api/place/photo?maxwidth=${maxWidth}&photo_reference=${imageItem.photo_reference}&key=${GOOGLE_API_KEY}`,
              isGoogleMedia: true,
            };
          }
        );
    ...
    if (item.isInAppPost) {
      ...
      return (
        <ImageBackground
          ...
          resizeMode="cover"
          source={{
            uri: `https://maps.googleapis.com/maps/api/place/photo?maxwidth=${Math.round(
              windowWidth
            )}&photo_reference=${item.photo_reference}&key=${GOOGLE_API_KEY}`,
          }}
        >
          <TouchableOpacity
            onPress={() => {
  ...
  const renderDoneButton = () => {
    return (
      <TouchableOpacity
        onPress={onMediaComponentDonePress}
        style={{
          padding: moderateScale(6),
          borderRadius: moderateScale(6),
          backgroundColor: colors.appPrimary,
      ...
      >
        <Text style={commonStyles.textWhite(14)}>Done</Text>
      </TouchableOpacity>
    );
  };
  ...
  const onLikeRestaurantPress = async () => {
    ...
    let objRestaurant = {
      ...
      restaurantImage: `https://maps.googleapis.com/maps/api/place/photo?maxwidth=${Math.round(
        windowWidth
      )}&photo_reference=${photoRef[0].photo_reference}&key=${GOOGLE_API_KEY}`,
    };
    console.log("🍽️ Saving restaurant from view:", restaurantDetails);
