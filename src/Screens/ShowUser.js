...
export default function ProfileScreen(props) {
    ...
    apiHandler
      ...
      .then((user) => {
        ...
        if (
          ...
          if (
            arrFollowers &&
            arrFollowers.length > 0 &&
            arrFollowers.findIndex((item, index) => {
    ...
    try {
      ...
      if (userDetails && userDetails.success === false && userDetails.message) {
        ...
        setCustomToastMessage(
          userDetails.message || "Failed to load user data"
        );
        setIsLoading(false);
        return;
      }
  ...
  const onFollowUnfollowPress = async () => {
    try {
      setIsLoading(true);
      isFollowing
        ? setLoaderTitle("Unfollowing ", userDetails.full_name)
        : setLoaderTitle("Following ", userDetails.full_name);
      let reqObj = {
        user_id: userID,
      };
      ...
      if (
        otherUserData &&
        otherUserData.success === false &&
        otherUserData.message
      ) {
        setShowErrorMessage(true);
        setCustomToastMessage(
          otherUserData.message || "Failed to refresh user data"
        );
        setIsLoading(false);
        return;
      }
      ...
      if (
        otherUserData &&
        (otherUserData.followers !== undefined ||
          otherUserData.following !== undefined)
      ) {
        ...
        // Refresh current user's profile to update following counter
    ...
    >
      ...
      <View style={commonStyles.flexFull}>
        <ScrollView>
          ...
          >
            <Ionicons
              onPress={onFollowUnfollowPress}
              name={isFollowing ? "remove-circle" : "add-circle"}
              style={{
                fontSize: moderateScale(20),
                color: isFollowing ? `#aa0000` : "#0aa120",
                position: "absolute",
                right: 0,
                top: 0,
                zIndex: 7,
          ...
          <Text
            onPress={onBioPress}
            numberOfLines={isBioExpanded ? 30 : 2}
            style={commonStyles.textWhite(16, {
              fontWeight: "600",
